(function ()
{
    ...
    /** @ngInject */
    function ToolbarController($rootScope, $q, $state, $timeout, $mdSidenav, $translate, $mdToast, msNavigationService, $scope, ref, data)
    {
        var vm = this;
        ...
        firebase.auth().onAuthStateChanged(function (user) {
            $timeout(function () {
                if (user) {
                    ...
                    var userSnapshot = ref.user.onSnapshot(function (doc) {
                        if (doc.exists) {
                            var user = doc.data();
                            $timeout(function () {
                                vm.user = user;
        ...
        $scope.$watch(function () {
            ...
            $timeout(function() {
                if (NewValue) {
                    console.log('data.toolbar', data.toolbar);
                    vm.toolbarTitle = NewValue.title;
                    vm.toolbarImg = NewValue.img;
                } else {
                    vm.toolbarTitle = 'Management';
                }
            });
        });
    }
